#!/usr/bin/env python3
"""
Video Lane Detection Inference Script (Fixed Version)
Processes video files frame by frame and outputs lane detection results
"""
...
def process_video(model, input_video, output_video, device='cuda'):
    ...
    with tqdm(total=total_frames, desc="Processing frames") as pbar:
        while True:
            ...
            img_pil = transforms.ToPILImage()(frame_rgb)
            ...
            # Draw lanes on frame
            result_frame = draw_lanes(frame, coords)
    ...
    print(f"Detected lanes in last frame: {len(coords)}")

def main():
    parser = argparse.ArgumentParser(description='Video Lane Detection Inference (Fixed)')
    ...
    # Process video
    process_video(model, args.input, args.output, args.device)
